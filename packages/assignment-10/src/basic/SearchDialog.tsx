...
// 1. TODO: 이 코드를 개선해서 API 호출을 최소화 해보세요 + Promise.all이 현재 잘못 사용되고 있습니다. 같이 개선해주세요.
// 캐시를 구현한 API 호출 함수
const createCachedFetch = (
  ...
  return async () => {
    // 캐시가 없을 때만
    if (!cache) {
      console.log(`API Call ${fetchFn.name}`, performance.now());
      cache = await fetchFn(); // 실제 API 호출,
    }
    return cache; // 그 외에는 캐시 결과 반환
  };
};

// 캐시된 API 호출 함수 생성 (각 API 호출에 대한 캐시된 버전 생성)
...
const fetchAllLectures = async () => {
  const start = performance.now(); // 호출 시간 측정
  console.log("API 호출 시작: ", start);

  // Promise.all을 사용해서 여러 API를 병렬로 호출함
  ...
  return results.flatMap((result) => result.data); // 결과를 평탄화(flatMap)하여 단일 배열로 반환함
};
...
const SearchDialog = ({ searchInfo, onClose }: Props) => {
  ...
  useEffect(() => {
    ...
    fetchAllLectures().then((results) => {
      ...
      console.log("모든 API 호출 완료 ", end);
      console.log("API 호출에 걸린 시간(ms): ", end - start);
      setLectures(results.flatMap((result) => result)); // fetchAllLectures에서 이미 result.data로 반환하고 있기때문에 그 result.data가 여기선 result임. setLectures로 상태 저장
    });
  }, []);
